...
if(args.dataset == 'cifar10'):
    ...
    sys.stdout.write("| ")
    trainset = torchvision.datasets.CIFAR10(root='./data', train=True, download=False, transform=transform_attack_train)
    testset = torchvision.datasets.CIFAR10(root='./data', train=False, download=False, transform=transform_attack_test)
    num_classes = 10
elif(args.dataset == 'cifar100'):
    print("| Preparing CIFAR-100 dataset...")
    sys.stdout.write("| ")
    trainset = torchvision.datasets.CIFAR100(root='./data', train=True, download=False, transform=transform_attack_train)
    testset = torchvision.datasets.CIFAR100(root='./data', train=False, download=False, transform=transform_attack_test)
    num_classes = 100
...
#net = Wide_ResNet_orthognal(args.depth, args.widen_factor, args.dropout, num_classes).cuda()
...
def train_clean(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(t):
        ...
        out_mixed, lam, mix_consist = net.dynamic_aggregate(x_a,x_b,alpha=args.alpha,noise_std=args.noise)
